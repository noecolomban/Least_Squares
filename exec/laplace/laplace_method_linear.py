...
T_values = [1000, 2000, 5000, 10000, 20000, 50000, 100000, 200000]  # Different T values to compare

changing_dim = lambda T, alpha: 0.01 * T  # Example of changing dimension with T, adjust as needed
...
for i, alpha in enumerate(list_alphas):
    color = colors[i]  # Use predefined color for each alpha
    difference = [(all_laplace_variances[(alpha, T)] - all_diagonal_variances[(alpha, T)]) for T in T_values]
    plt.plot(T_values, difference, label=f"Variance Difference (Laplace - Diagonal) for alpha={alpha:.2f}", marker='o', color=color)
plt.xscale("log")
#plt.yscale("log")
plt.xlabel("T")
plt.ylabel("Variance Difference")
plt.title(f"Variance Difference Trajectories for Different Alphas, dim={dim_text}")
plt.legend()
plt.grid()
plt.savefig(f"images/variance_difference_trajectories_different_alphas_linear_schedule_alpha_max={max(list_alphas)}_{dim_text}_Tmax={max(T_values)}.pdf")
plt.show()
