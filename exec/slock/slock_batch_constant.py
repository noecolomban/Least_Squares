...
def batch_factor_constant(batch, alpha, beta):
    '''So that eta_star is batch_factor_constant * best_slock_eta(T) for the given batch size.'''
    return batch**((alpha)/(2*alpha+beta-1))
...
eta = 0.001
slock_constant = SlockConstant(model, x0, T_max=max(T_values), optimize=optimize, base_lr=eta)
mode = Mode.SLOCK
#%%
...
alpha = 1.1
biases, variances = {}, {}
slock_constant._update_model_for_alpha(alpha)  # Update model for the specific alpha

for batch in batches:
    biases[batch], variances[batch] = {}, {}
    for T in T_values:
        print(f"Computing biases and variances for batch size {batch} and T={T}...")
        eta_star = batch_factor_constant(batch, alpha, beta)*slock_constant.compute_best_slock_eta(T, m_constant=Delta)
        slock_constant._setup_for_T(T, base_lr=eta_star)  # Update schedule for new T
        bias, variance = slock_constant.compute_slock_biases_and_variances([T], batch=batch, K=1)
...
plt.yscale('log')
plt.grid()
plt.savefig(f"images/slock/CONSTANT_BATCH_comparison_batch={batches}_alpha={alpha}.pdf")
plt.show()
# %%
...
plt.yscale('log')
plt.grid()
plt.savefig(f"images/slock/CONSTANT_BATCH_comparison_risk_batch={batches}_alpha={alpha}.pdf")
plt.show()
# %%
...
for batch in batches:
    ...
    for alpha in list_alphas:
        ...
        slock_constant._update_model_for_alpha(alpha)  # Update model for the specific alpha
        eta_star = (
            slock_constant.compute_best_slock_eta(T, m_constant=Delta)
            * batch_factor_constant(batch, alpha, beta)
        )
        slock_constant._setup_for_T(T, base_lr=eta_star)  # Update schedule for new T
        bias, variance = slock_constant.compute_slock_biases_and_variances([T], batch=batch, K=1)
...
plt.ylabel("Bias")
plt.title(f"(CST) Slock Bias for Different Batch Sizes; b*T={bT}, eta_star")
plt.legend()
plt.xscale('log')
plt.yscale('log')
plt.grid()
plt.savefig(f"images/slock/CONSTANT_BATCH_comparison_bias_bT={bT}_eta_star_for_each_batch.pdf")
plt.show()
# %%
#Plot variance
plt.figure(figsize=(12, 8))
...
plt.yscale('log')
plt.grid()
plt.savefig(f"images/slock/CONSTANT_BATCH_comparison_variance_bT={bT}_eta_star_for_each_batch.pdf")
plt.show()
# %%
#Plot risk
plt.figure(figsize=(12, 8))
...
plt.yscale('log')
plt.grid()
plt.savefig(f"images/slock/CONSTANT_BATCH_comparison_risk_bT={bT}_eta_star_for_each_batch.pdf")
plt.show()
...
for batch in batches:
    ...
    for alpha in list_alphas:
        ...
        slock_constant._setup_for_T(T, base_lr=eta_star)  # Update schedule for new T
        bias = slock_constant.compute_slock_approx_bias(T, T, m_constant=Delta, m_exponent=beta, batch=batch)
        variance = slock_constant.compute_slock_approx_variance(T, T, batch=batch)
...
plt.yscale('log')
plt.grid()
plt.savefig(f"images/slock/CONSTANT_BATCH_comparison_approx_bias_bT={bT}_eta_star_for_each_batch.pdf")
plt.show()
...
plt.yscale('log')
plt.grid()
plt.savefig(f"images/slock/CONSTANT_BATCH_comparison_approx_variance_bT={bT}_eta_star_for_each_batch.pdf")
plt.show()
...
plt.yscale('log')
plt.grid()
plt.savefig(f"images/slock/CONSTANT_BATCH_comparison_approx_risk_bT={bT}_eta_star_for_each_batch.pdf")
plt.show()  
# %%
...
for alpha in alphas:
    slock_constant._update_model_for_alpha(alpha)  # Update model for the specific alpha
    slock_constant._setup_for_T(T, base_lr=0.001)  # Setup for T with a placeholder learning rate
    best_etas[alpha] = slock_constant.compute_best_slock_eta(T, m_constant=Delta)
    exact_etas[alpha] = slock_constant.compute_exact_eta(T, m_constant=Delta)
# %%
