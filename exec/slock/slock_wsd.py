...
approx_variance, exact_variance, approx_bias, exact_bias = (
...
for alpha in list_alphas:
    ...
    Y_slock = np.array([approx_variance[(alpha, T)] for T in T_values])
    Y_diagonal = np.array([exact_variance[(alpha, T)] for T in T_values])
    plt.plot(T_values, Y_slock, label=f"equivalent Variance (alpha={alpha})", marker='o', color=color)
    plt.plot(T_values, Y_diagonal, label=f"exact Variance (alpha={alpha})", marker='x', linestyle='--', color=color)
plt.xscale('log')
plt.yscale('log')
...
for alpha in list_alphas:
    color = colors[list_alphas.index(alpha)]
    Y_slock = np.array([approx_bias[(alpha, T)] for T in T_values])
    Y_diagonal = np.array([exact_bias[(alpha, T)] for T in T_values])
    plt.plot(T_values, Y_slock, label=f"equivalent Bias (alpha={alpha})", marker='o', color=color)
    plt.plot(T_values, Y_diagonal, label=f"exact Bias (alpha={alpha})", marker='x', linestyle='--', color=color)
plt.xscale('log')
plt.yscale('log')
plt.xlim(100, max(T_values))
plt.xlabel("T (log scale)")
...
#%%
ratios_variance = {key: approx_variance[key] / exact_variance[key] for key in approx_variance.keys()}
ratios_bias = {key: approx_bias[key] / exact_bias[key] for key in approx_bias.keys()}
...
plt.xlabel("T (log scale)")
plt.ylabel("Variance (log scale)")
plt.title(f"Variance Trajectories for Different Alphas (equivalent vs exact) eta={eta}")
plt.legend()
plt.grid()
...
plt.title(f"Bias Ratios for Different Alphas (equivalent vs exact) eta={eta}")
plt.legend()
plt.grid()
...
for alpha in list_alphas:
    ...
    plt.ylabel("Risk")
    plt.yscale('log')
    plt.title(f"Risk for Different cooldown lengths (equivalent vs exact) eta={eta}, alpha={alpha}")
    plt.legend()
    plt.grid()
